...
class OpenWindow extends calculator{
	...
	public void makeFrame() {
		...
		String btnSText[] = {"C", "="} ;						// This is what's inside the Button btnS.
		
		Button btnS[] = new Button[2] ;							// Create a Button btnS in a 2 X 1 array.
		
		String[][] btnText= {{"7", "8", "9", "÷"},
							{"4", "5", "6", "X"},
							{"1", "2", "3", "-"},
							{".", "0", "<", "+"}};				// This is what's inside the Button btn.
		
		Button[][] btn = new Button[4][4] ; 					// Create a Button btn in a 4 x 4 array.
	    ...
	    Panel head = new Panel() ;								// Create a Panel head
	    Panel panel = new Panel() ;								// Create a Panel panel
	    
	    head.setPreferredSize(new Dimension(360, 85));			// Adjust the size of the Panel panel to width 360px, height 85px.
		...
		Panel head1 = new Panel() ;								// Create a Panel head1, head2
		Panel head2 = new Panel() ;								// head1 is JTextField l
																// head2 is Button btnS, {"C", "="}
	    ...
	    for(int i = 0; i < 2; i++) {
			...
			head2.add(btnS[i]) ;								// Add Button btnS to head2
		}
	    ...
	    for(int j = 0; j < 4; j++) {
			for(int i = 0; i < 4; i++) {
				...
				panel.add(btn[j][i]) ;							// Add Button btn to panel
			}	    
	    }
